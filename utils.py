...
def get_exp(folder):
    Exp = namedtuple('Exp', ['config', 'train_df', 'valid_df', 'test_df'])
    config = inirama.Namespace()
    config.read("%s/config.ini" % folder)
    print config['DEFAULT']
    train_df = pandas.read_csv('%s/train.log' % folder, sep='\t')
    valid_df, test_df = None, None
    if os.path.isfile("%s/valid.log" % folder):
        print 'valid_df'
        valid_df = pandas.read_csv('%s/valid.log' % folder, sep='\t')
    if os.path.isfile("%s/test.log" % folder):
        print 'test_df'
        test_df = pandas.read_csv('%s/test.log' % folder, sep='\t')
    return Exp(config, train_df, valid_df, test_df)
